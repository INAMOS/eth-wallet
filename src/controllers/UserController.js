...
module.exports = {
  ...
  postSignUp: function(req, res, next) {
    ...
    db.query(
      `SELECT email FROM usuarios WHERE email='${req.body.email}'`,
      (err, row, fields) => {
        if (err) throw err;
        ...
        if (row[0].email == req.body.email) {
          req.flash(
            "ErrorEmail",
            "Email en uso por favor introduzca otro correo"
          );
          res.redirect("/signup");
        } else {
          ...
          db.query("INSERT INTO usuarios SET ?", user, (err, rows, fields) => {
            ...
            db.query(
              `SELECT id FROM usuarios WHERE direccion='${account.address}'`,
              (er, row, field) => {
                private_key.ide_usu = row[0].id;

                db.query(
                  "INSERT INTO llaves_privadas SET ?",
                  private_key,
                  (error, rowss, fieldss) => {
