async function proccesFiles(files, db) {
  try {
    ...
    let firstPromise = uploadFile(file1, db);
    let secondPromise = uploadFile(file2, db);
    let thirdPromise = uploadFile(file3, db);
  ...
  return new Promise((resolve, reject) => {
    ...
    file.mv(__dirname + `/../.././views/Module/Files/${file.name}`, err => {
      if (err) {
        ...
        db.query(
          `LOAD DATA INFILE '${route}/../.././views/Module/Files/${
            file.name
          }' IGNORE INTO TABLE ${name} FIELDS TERMINATED BY ',' LINES TERMINATED BY ';'`,
          (err, rows, fields) => {
            if (!err) resolve("Migracion realizada con exito");
            else
              reject(
                `Error al respaldar el archivo ${
                  file.name
                } , intentelo de nuevo`
              );
